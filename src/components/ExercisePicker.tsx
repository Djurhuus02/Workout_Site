        ...
        {filtered.length === 0 ? (
          ...
          [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([letter, exs]) => (
            <div key={letter}>
              ...
              {exs.map(ex => <ExerciseRow key={ex.id} exercise={ex} onSelect={onSelect} />)}
            </div>
          ))
        ) : (
          filtered.map(ex => <ExerciseRow key={ex.id} exercise={ex} onSelect={onSelect} />)
        )}
      </div>
    </div>
  )
...
function ExerciseRow({ exercise, onSelect }: { exercise: Exercise; onSelect: (e: Exercise) => void }) {
  ...
  return (
    <button
      ...
      className="w-full flex items-center justify-between gap-3 py-3 border-b border-gray-800 text-left hover:bg-gray-800/50 transition-colors rounded-lg px-2"
    >
